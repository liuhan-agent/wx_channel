...
**Goal:** 将 wx_channel 与 TrendRadar 微信视频号的有效采集上限统一到 `30 / 500 / 100 / 1000`，并用分页边界测试证明扩容不会破坏截断、重复 marker 和部分完成语义。

**Architecture:** wx_channel 批次协议继续负责请求边界，评论采集器沿用现有分页循环，只放宽微信批次 limits 校验并补充边界 fixture。TrendRadar 保留其它平台的通用 AdminLimits，通过平台专属容量合同为微信视频号选择 `30 / 500 / 100 / 1000`，配置页面在平台切换时同步微信专属 max，后端保存和运行前校验使用同一组值；批次验证器按最坏 30 个作品的评论和回复总量扩大文件预算。
...
- [ ] 添加批次 limits 边界测试：接受 `works=30, top_level_comments_per_work=500, replies_per_comment=100, replies_per_work=1000`，分别拒绝 501、101、1001。
...
- [ ] 将请求校验改为 `30 / 500 / 100 / 1000`，旧 limits 请求继续有效。
...
- [ ] 保留其它平台的通用 AdminLimits；为 `wechat_channels` 使用 `30 / 500 / 100 / 1000` 的专属边界。
- [ ] 后端接受微信边界值，拒绝 31、501、101、1001 对应的超限值；其它平台继续使用原有上限。
...
- [ ] 为评论、单评论回复、作品回复字段增加微信专属 max 数据属性，分别为 500、100、1000；作品数继续为 30。
...
- [ ] 更新文档，说明微信上限为 `30 / 500 / 100 / 1000`，其它平台不继承微信专属上限，并提示请求数、运行时间和存储占用会增加。
